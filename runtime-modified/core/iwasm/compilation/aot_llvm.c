...
// added for async io, create sigpoll_ptr
static bool
create_sigpoll_ptr(AOTCompContext * comp_ctx, AOTFuncContext * func_ctx)
{
    LLVMValueRef io_sigpending_ptr_offset=I32_EIGHT, io_sigpending_ptr_addr;
    if(!(io_sigpending_ptr_addr=LLVMBuildInBoundsGEP2(
        comp_ctx->builder,INT8_PTR_TYPE,func_ctx->exec_env, &io_sigpending_ptr_offset, 1,
        "io_sigpending_ptr_addr_ptr"
    ))){
        aot_set_last_error("llvm build in bounds gep failed");
        return false;
    }
    if(!(io_sigpending_ptr_addr=LLVMBuildBitCast(comp_ctx->builder, io_sigpending_ptr_addr,
                                                    INT64_PTR_TYPE, "io_sigpending_ptr_addr_ptr_cast"))){
        aot_set_last_error("llvm build bit cast failed");
        return false;
    }
    if(!(func_ctx->sigpoll_ptr=
        LLVMBuildLoad2(comp_ctx->builder, INT64_PTR_TYPE, io_sigpending_ptr_addr,"io_sigpending_ptr_addr"))){
        aot_set_last_error("llvm build LOAD failed");
        return false;
    }
    return true;
                        ...
                        AOTFunc *func, uint32 func_index)
{
    ...
    // added for async io
    if(!create_sigpoll_ptr(comp_ctx, func_ctx))
        goto fail;
