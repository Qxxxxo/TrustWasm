...
#include "bh_log.h"
#include "mem_alloc.h"
#include "../interpreter/tz_io_sigtable.h"
...
// added for async io
void aot_poll_ready_signal(WASMExecEnv * exec_env)
{
    ...
    if(get_ready_signal(&signo[0],&signo[1])==1){
        ...
        // dequeue & update start time, try to catch up io_helper handle req start
        dequeue(&io_queues[signo[0]],&req);
        //  EMA update exec time
        exec_time_table[req.type].b=(uint64_t)((float)(now-req.start)*0.91+(float)exec_time_table[req.type].b*0.01);
        // update wait profile
...
#endif
        // since TA is single thread, we don't need mutex
        AOTModuleInstance * module_inst = (AOTModuleInstance *)wasm_runtime_get_module_inst(exec_env);
        AOTFunctionInstance * handler_func=(AOTFunctionInstance *)io_sigtable[signo[0]][signo[1]].function;
        // patch function pointer in case of reassignment
        uint32 func_idx = io_sigtable[signo[0]][signo[1]].func_idx;
        uint32 func_type_idx = ((uint32*)module_inst->func_type_indexes.ptr)[func_idx];
        handler_func->u.func.func_type=((AOTModule *)module_inst->aot_module.ptr)->func_types[func_type_idx];
        handler_func->u.func.func_ptr = ((void **)module_inst->func_ptrs.ptr)[func_idx];
        // DMSG("call handler");
        temp_unset_all_io_signal_pending();
        wasm_runtime_call_wasm(exec_env,handler_func,2,signo);
        temp_recover_all_io_signal_pending();
        // must poll one time to update io sigpending
...
aot_get_indirect_function (AOTModuleInstance *module_inst, uint32 tbl_idx,
  ...
  tbl_elem_val = ((uint32*)tbl_inst->data)[table_elem_idx];
