...
deploywatz () {
    ...
    cd $ROOT_DIR/build
    rsync --progress -r $BM_BUILDER_HOSTNAME:$BM_BUILDER_PATH/../runtime-modified/product-mini/platforms/linux-trustzone/vedliot_attester/out/\* out/
    sshpass -p "$BM_BOARD_USER" rsync -e 'ssh -p 22 -o StrictHostKeyChecking=no' --progress out/ca/vedliot_attester $BM_BOARD_USER@$BM_BOARD_HOSTNAME:/usr/bin
    sshpass -p "$BM_BOARD_USER" rsync -e 'ssh -p 22 -o StrictHostKeyChecking=no' --progress out/ta/bc20728a-6a28-49d8-98d8-f22e7535f137.ta $BM_BOARD_USER@$BM_BOARD_HOSTNAME:/lib/optee_armtz
    sshpass -p "$BM_BOARD_USER" ssh $BM_BOARD_USER@$BM_BOARD_HOSTNAME $BM_QEMU_SSH_ARGS 'chmod 666 /lib/optee_armtz/bc20728a-6a28-49d8-98d8-f22e7535f137.ta'

    announcedeploy "WaTZ verifier"
    cd $ROOT_DIR/build
    rsync --progress -r $BM_BUILDER_HOSTNAME:$BM_BUILDER_PATH/../runtime-modified/product-mini/platforms/linux-trustzone/vedliot_verifier/out/\* out/
    sshpass -p "$BM_BOARD_USER" rsync -e 'ssh -p 22 -o StrictHostKeyChecking=no' --progress out/ca/vedliot_verifier $BM_BOARD_USER@$BM_BOARD_HOSTNAME:/usr/bin
    sshpass -p "$BM_BOARD_USER" rsync -e 'ssh -p 22 -o StrictHostKeyChecking=no' --progress out/ta/526461e2-d34a-4d96-8ca3-7fb9f4b898ef.ta $BM_BOARD_USER@$BM_BOARD_HOSTNAME:/lib/optee_armtz
    sshpass -p "$BM_BOARD_USER" ssh $BM_BOARD_USER@$BM_BOARD_HOSTNAME $BM_QEMU_SSH_ARGS 'chmod 666 /lib/optee_armtz/526461e2-d34a-4d96-8ca3-7fb9f4b898ef.ta'
}
