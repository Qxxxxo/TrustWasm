...
announcedeploy "async_promise app"
cd $SCRIPT_DIR
rsync --progress -r $BM_BUILDER_HOSTNAME:$BM_BUILDER_PATH/async_promise/app/\*.wasm out/
rsync --progress -r $BM_BUILDER_HOSTNAME:$BM_BUILDER_PATH/async_promise/app/\*.aot out/
