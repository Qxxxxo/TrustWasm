...
int main(int argc, char **argv)
{
	for(int k=0;k<RUN_BENCHMARK_TIMES;k++){
		...
		uint32_t signed_null=sign_ptr(NULL,SIGNATURE,TAG);
		...
		if (mpu6050_init(MPU6050_I2C_NO, MPU6050_ADDR) == 0)
		{
			...
			for(int i=0;i<BENCHMARK_ITERATIONS;i++){
				if(write_secure_future>=0){
					// get write result
					storage_res=get_async_data_future(AT24CXX_PERIPH_NO,write_secure_future,0,0);
				}
				if(mpu6050_future>=0){	
					res=get_async_data_future(MPU6050_PERIPH_NO,mpu6050_future,signed_data,signed_len);
				}
				...
				if(res>0){
					...
					write_secure_future=at24cxx_write_async_future(AT24CXX_I2C_NO,AT24CXX_ADDR,0x00,signed_data,MPU6050_DATA_LEN+1);
				}else{
					write_secure_future=-1;
					printf("get async data sample failed with %x\n",res);
				}
				native_wait(wait_ms);
			}
			if(write_secure_future>=0){
