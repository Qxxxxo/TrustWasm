		...
		if (mpu6050_init(MPU6050_I2C_NO, MPU6050_ADDR) == 0)
		{
			...
			for(int i=0;i<BENCHMARK_ITERATIONS;i++){
				...
				if(res>=0){
					...
					// write mpu6050 data and detect result to storage here use eeprom
					at24cxx_write(AT24CXX_I2C_NO,AT24CXX_ADDR,0x00,signed_data,MPU6050_DATA_LEN+1);
				}else{
					printf("sample failed with %x\n",res);
				}
				native_wait(wait_ms);
			}
