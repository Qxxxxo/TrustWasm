...
int main(int argc, char **argv)
{
	for(int k=0;k<RUN_BENCHMARK_TIMES;k++){
		...
		uint32_t signed_raw_alt=sign_ptr(raw_alt,SIGNATURE,TAG);
		uint32_t signed_mpu_data=sign_ptr(mpu_data,SIGNATURE,TAG);
		uint32_t signed_save_str=sign_ptr(save_str,SIGNATURE,TAG);
		uint32_t signed_time_len=sign_ptr(&utc_time_len,SIGNATURE,TAG);
		uint32_t signed_lon_len=sign_ptr(&lon_len,SIGNATURE,TAG);
		uint32_t signed_lat_len=sign_ptr(&lat_len,SIGNATURE,TAG);
		uint32_t signed_alt_len=sign_ptr(&raw_alt_len,SIGNATURE,TAG);
		...
		if (tel0157_init(TEL0157_I2C_NO, TEL0157_ADDR, TEL0157_GNSS_MODE, TEL0157_RGB) == 0&&mpu6050_init(MPU6050_I2C_NO,MPU6050_ADDR)==0)
		{
			...
			for(int i=0;i<BENCHMARK_ITERATIONS;i++){
				...
				res=get_async_data_future(TEL0157_PERIPH_NO,utc_time_future,signed_time,signed_time_len);
				if(res<0){
					printf("get async data utc time failed with %x\n",res);
				}
				res=get_async_data_future(TEL0157_PERIPH_NO,lon_future,signed_lon_lat,signed_lon_len);
				if(res<0){
				...
				res=get_async_data_future(TEL0157_PERIPH_NO,lat_future,signed_lon_lat,signed_lat_len);
				if(res<0){
				...
				res=get_async_data_future(TEL0157_PERIPH_NO,alt_future,signed_raw_alt,signed_alt_len);
				if(res<0){
				...
				res=get_async_data_future(MPU6050_PERIPH_NO,mpu_future,signed_mpu_data,signed_mpu_len);
				if(res<0){
				...
				write_future=at24cxx_write_async_future(AT24CXX_I2C_NO,AT24CXX_ADDR,0x00,signed_save_str,SAVE_STRING_LEN);
				native_wait(wait_ms);
			}
