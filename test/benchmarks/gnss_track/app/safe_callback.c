...
static void init(ekf_t * mekf)
{
    ...
    // position
    mekf->x[0] = -2.168816181271560e+006;
    mekf->x[1] =  4.386648549091666e+006;
    mekf->x[2] =  4.077161596428751e+006;
...
uint32_t signed_time;
uint32_t signed_lon_lat;
uint32_t signed_raw_alt;
uint32_t signed_mpu_data;
uint32_t signed_save_str;
uint32_t signed_time_len;
uint32_t signed_lon_len;
uint32_t signed_lat_len;
uint32_t signed_alt_len;
...
int mpu6050_handler(uint32_t signo, uint32_t sub_signo){
    ...
    res=at24cxx_write_async(AT24CXX_I2C_NO,AT24CXX_ADDR,0x00,signed_save_str,SAVE_STRING_LEN,at24cxx_handler);
    if(res<0){
...
int tel0157_time_handler(uint32_t signo, uint32_t sub_signo){
    utc_time_len=sizeof(tel0157_time_t);
    signed_time=sign_ptr(&time,SIGNATURE,TAG);
    signed_time_len=sign_ptr(&utc_time_len,SIGNATURE,TAG);
    int res=get_async_data(signo,sub_signo,signed_time,signed_time_len);
    if(res<0){
...
int tel0157_lon_handler(uint32_t signo, uint32_t sub_signo){
    lon_len=sizeof(tel0157_lon_lat_t);
    signed_lon_lat=sign_ptr(&lon_lat,SIGNATURE,TAG);
    signed_lon_len=sign_ptr(&lon_len,SIGNATURE,TAG);
    int res=get_async_data(signo,sub_signo,signed_lon_lat,signed_lon_len);
    if(res<0){
...
int tel0157_lat_handler(uint32_t signo, uint32_t sub_signo){
    lat_len=sizeof(tel0157_lon_lat_t);
    signed_lon_lat=sign_ptr(&lon_lat,SIGNATURE,TAG);
    signed_lat_len=sign_ptr(&lat_len,SIGNATURE,TAG);
    int res=get_async_data(signo,sub_signo,signed_lon_lat,signed_lat_len);
    if(res<0){
...
int tel0157_alt_handler(uint32_t signo, uint32_t sub_signo){
    raw_alt_len=3;
    signed_raw_alt=sign_ptr(raw_alt,SIGNATURE,TAG);
    signed_alt_len=sign_ptr(&raw_alt_len,SIGNATURE,TAG);
    int res=get_async_data(signo,sub_signo,signed_raw_alt,signed_alt_len);
    if(res<0){
    ...
    for(int k=0;k<RUN_BENCHMARK_TIMES;k++){
        ...
        if (tel0157_init(TEL0157_I2C_NO, TEL0157_ADDR, TEL0157_GNSS_MODE, TEL0157_RGB) == 0&&mpu6050_init(MPU6050_I2C_NO,MPU6050_ADDR)==0)
        {
            ...
            for(int i=0;i<BENCHMARK_ITERATIONS;i++){
                // send async request
                res=tel0157_get_utc_time_async(TEL0157_I2C_NO, TEL0157_ADDR,tel0157_time_handler);
                if(res<0){
                    printf("tel0157_get_utc_time_async failed with %x\n",res);
                }
                res=tel0157_get_lon_async(TEL0157_I2C_NO, TEL0157_ADDR,tel0157_lon_handler);
                if(res<0){
                    printf("tel0157_get_lon_async failed with %x\n",res);
                }
                res=tel0157_get_lat_async(TEL0157_I2C_NO, TEL0157_ADDR,tel0157_lat_handler);
                if(res<0){
                    printf("tel0157_get_lat_async failed with %x\n",res);
                }
                res=tel0157_get_alt_async(TEL0157_I2C_NO, TEL0157_ADDR,tel0157_alt_handler);
                if(res<0){
                    printf("tel0157_get_alt_async failed with %x\n",res);
                }
