    ...
    for(int k=0;k<RUN_BENCHMARK_TIMES;k++){
        ...
        if (tel0157_init(TEL0157_I2C_NO, TEL0157_ADDR, TEL0157_GNSS_MODE, TEL0157_RGB) == 0&&mpu6050_init(MPU6050_I2C_NO,MPU6050_ADDR)==0)
        {
            ...
            for(int i=0;i<BENCHMARK_ITERATIONS;i++){
                if(tel0157_get_utc_time(1,0x20,signed_time)!=0){
                    printf("tel0157 get utc time failed\n");
                }
                if(tel0157_get_lon(TEL0157_I2C_NO, TEL0157_ADDR,signed_lon_lat)==0){
                    longitude=(double)lon_lat.lonDD + (double)lon_lat.lonMM/60.0 + (double)lon_lat.lonMMMMM/100000.0/60.0;
                }else{
                    printf("tel0157 get lon failed\n");
                }
                if(tel0157_get_lat(TEL0157_I2C_NO, TEL0157_ADDR,signed_lon_lat)==0){
                    latitude=(double)lon_lat.latDD + (double)lon_lat.latMM/60.0 + (double)lon_lat.latMMMMM/100000.0/60.0;
                }else{
                    printf("tel0157 get lat failed\n");
                }
                if(tel0157_get_alt(TEL0157_I2C_NO, TEL0157_ADDR,signed_raw_alt)==0){
                    altitude=(double)((uint16_t)(raw_alt[0]&0x7F)<<8|raw_alt[1])+(double)raw_alt[2]/100.0;
                    if(raw_alt[0]&0x80){
                        altitude=-altitude;
                    }
                }else{
                    printf("tel0157 get alt failed\n");
                }
                if(mpu6050_sample(MPU6050_I2C_NO,MPU6050_ADDR,signed_mpu_data)==0){
                ...
                at24cxx_write(AT24CXX_I2C_NO,AT24CXX_ADDR,0x00,signed_save_str,SAVE_STRING_LEN);
                native_wait(wait_ms);
            }
