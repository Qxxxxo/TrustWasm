...
void print_overhead(uint64_t overhead[BENCHMARK_ITERATIONS][OVERHEAD_POINTS], uint64_t * pta_timestamps){
    for(int i=0;i<BENCHMARK_ITERATIONS-1;i++){
        ...
        pta_timestamps[IO_HELPER_SERVICE_OVERHEAD_BENCHMARK_POINTS*(i+1)]-overhead[i][2],
        overhead[i][3]-pta_timestamps[IO_HELPER_SERVICE_OVERHEAD_BENCHMARK_POINTS*(i+1)+1],
        overhead[i][1]-overhead[i][0],
        pta_timestamps[IO_HELPER_SERVICE_OVERHEAD_BENCHMARK_POINTS*i+2]-overhead[i][1],
        overhead[i][2]-pta_timestamps[IO_HELPER_SERVICE_OVERHEAD_BENCHMARK_POINTS*i+3]);
    }
}
...
int main(int argc, char **argv)
{
	for(int k=0;k<RUN_BENCHMARK_TIMES;k++){
		...
		uint32_t read_len=LZ4_FRAME_SIZE;
		...
		read_future=i2c_read_bytes_async_future(TEST_I2C_NO,TEST_I2C_ADDR,0,LZ4_FRAME_SIZE);
		for(int i=0;i<BENCHMARK_ITERATIONS;i++){
            ...
            get_benchmark_time(4,(uint32_t)&(overhead[i][2]));
            get_benchmark_time(5,(uint32_t)&(overhead[i][3]));
		}
