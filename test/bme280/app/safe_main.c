 ...
 * refer to
 * https://github.com/libdriver/bme280/blob/main/src/driver_bme280.c
...
int bme280_init(uint32_t i2c_no,uint32_t i2c_addr){
    ...
    uint32_t signed_config=sign_ptr(config,SIGNATURE,TAG);
    res=i2c_read_regs(i2c_no,i2c_addr,0,BME280_REG_ID,signed_id,1);
    // printf("BME280 get id %x, res: %x\n",id,res);
    if(res!=0) return res;
    if(id!=0x60) return -1;
    config[0]=BME280_REG_CTRL_HUM;
    config[1]=0x01;
    res=i2c_write_bytes(i2c_no,i2c_addr,0,signed_config, 2);
...
int bme280_get_calibration(uint32_t i2c_no,uint32_t i2c_addr, bme280_calibration_data_t * calib_data){
    int res;
    uint8_t data[24];
    uint32_t signed_buf=sign_ptr(data,SIGNATURE,TAG);
    res=i2c_read_regs(i2c_no,i2c_addr,0,BME280_CALIBRATION_REG_TEMP_PRESS,signed_buf,24);
    if(res!=0) return res;
    ...
    // hum
    res=i2c_read_regs(i2c_no,i2c_addr,0,BME280_CALIBRATION_REG_HUM_1,signed_buf,1);
    if(res!=0) return res;
    calib_data->dig_H1=data[0];
    res=i2c_read_regs(i2c_no,i2c_addr,0,BME280_CALIBRATION_REG_HUM,signed_buf,8);
    if(res!=0) return res;
...
int bme280_sample(uint32_t i2c_no,uint32_t i2c_addr, uint8_t buf[8]){
    ...
    res=i2c_read_regs(i2c_no,i2c_addr,0,BME280_REG_PRESS_MSB,signed_buf,8);
    return res;
}
...
{
    uint8_t buf[8];
    bme280_calibration_data_t calib_data;
    int32_t t_fine;
    float temperature, pressure, humidity;
    bme280_init(BME280_I2C_NO,BME280_ADDR);
    bme280_get_calibration(BME280_I2C_NO, BME280_ADDR,&calib_data);
    bme280_sample(BME280_I2C_NO,BME280_ADDR,buf);
    temperature = bme280_convert_temperature(buf, &calib_data,&t_fine);
    pressure = bme280_convert_pressure(buf, &calib_data,t_fine);
    humidity = bme280_convert_humidity(buf, &calib_data,t_fine);
